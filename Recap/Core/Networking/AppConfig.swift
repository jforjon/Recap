...
enum AppConfig {
    ...
    // API_HOST is deliberately gone. Summaries are generated by calling
    // Anthropic directly from the device with the user's own key — see
    // `AnthropicClient` — so the app depends on no server of ours at all.
}
